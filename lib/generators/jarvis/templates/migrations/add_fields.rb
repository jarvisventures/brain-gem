class AddFields < ActiveRecord::Migration<%= Rails::VERSION::MAJOR >= 5 ? "[#{Rails.version.to_f}]" : "" %>
  def change
    ...
    add_column(:user_tag, :brain_token, :string) unless column_exists?(:user_tag, :brain_token)
    add_index(:user_tag, :brain_token, unique: true) unless index_exists?(:user_tag, :brain_token)
    ...
    add_column(:email_tag, :brain_token, :string) unless column_exists?(:email_tag, :brain_token)
    add_index(:email_tag, :brain_token, unique: true) unless index_exists?(:email_tag, :brain_token)
